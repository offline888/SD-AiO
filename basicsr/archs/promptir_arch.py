...
class PromptIR(nn.Module):
    def __init__(
        ...
        self.decoder_level3 = PromptIRBlock(
            int(dim * 2**2), heads[2], ffn_expansion_factor, bias, num_blocks[2]
        )
        ...
        self.decoder_level2 = PromptIRBlock(
            int(dim * 2**1), heads[1], ffn_expansion_factor, bias, num_blocks[1]
        )
        ...
        self.decoder_level1 = PromptIRBlock(
            int(dim * 2**1), heads[0], ffn_expansion_factor, bias, num_blocks[0]
        )

        self.refinement = PromptIRBlock(
    ...
    def get_layer_id_and_scale_exp(self, para_name):
        N = (
            self.num_blocks[0] + self.num_blocks[1] + self.num_blocks[2]
        ) * 2 + self.num_blocks[3]
        if para_name.startswith("encoder_level") or para_name.startswith("down"):
            stage_id = int(para_name.split(".")[0][-1])
            layer_id = stage_id - 1
        elif (
            para_name.startswith("decoder_level")
            or para_name.startswith("up")
            or para_name.startswith("reduce_chan_level")
            or para_name.startswith("reduce_noise_level")
        ):
            stage_id = int(para_name.split(".")[0][-1])
            layer_id = stage_id + 3
        elif para_name.startswith("latent"):
            layer_id = 3
        elif para_name.startswith("prompt") or para_name.startswith("noise_level"):
            stage_id = int(para_name.split(".")[0][-1])
            layer_id = stage_id + 3
        elif para_name.startswith("patch_embed"):
            layer_id = 0
        else:
            layer_id = 7

        return layer_id, 7 - layer_id
    ...
    def forward(self, inp_img):
        ...
        if self.decoder:
            # dec3_param = checkpoint.checkpoint(self.prompt3, latent)
        ...
        if self.decoder:
            # dec2_param = checkpoint.checkpoint(self.prompt2, out_dec_level3)
        ...
        if self.decoder:
            # dec1_param = checkpoint.checkpoint(self.prompt1, out_dec_level2)
