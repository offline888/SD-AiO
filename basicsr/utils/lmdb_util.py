...
def make_lmdb_from_imgs(
    ...
    if map_size is None:
        ...
        if save_byte:
            ...
            data_size_per_img = img.size * 255
        print("Data size per image is: ", data_size_per_img)
        data_size = data_size_per_img * len(img_path_list)
        map_size = data_size * 5

    print("Try to allocate mem: ", map_size)
    env = lmdb.open(lmdb_path, map_size=map_size)

    # write data to lmdb
    pbar = tqdm(total=len(img_path_list), unit="it")
    ...
    for idx, (path, key) in enumerate(zip(img_path_list, keys)):
        ...
        if save_byte:
            ...
            img = cv2.imread(osp.join(data_path, path), cv2.IMREAD_UNCHANGED).astype(
                np.uint16
            )
            h, w, c = img.shape
            img_byte = np.append(np.array([h, w, c]).astype(img.dtype), img).tobytes()
