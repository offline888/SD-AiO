...
from src.flux2.embeddings import (
    TimestepEmbedding,
    Timesteps,
    apply_rotary_emb,
    get_1d_rotary_pos_embed,
)
from src.models.modulation import FLUX2ModulationV2
...
class Flux2TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # Ensure dtypes match between hidden_states and encoder_hidden_states
...
class Flux2Transformer2DModel(
    ...
    def __init__(
        ...
        self.double_stream_modulation_img = FLUX2ModulationV2(dim=self.inner_dim, mod_param_sets=2, bias=False)
        self.double_stream_modulation_txt = Flux2Modulation(self.inner_dim, mod_param_sets=2, bias=False)
        # Only one set of modulation parameters as the attn and FF sub-blocks are run in parallel for single stream
        self.single_stream_modulation = FLUX2ModulationV2(dim=self.inner_dim, mod_param_sets=1, bias=False)
    ...
    def forward(
        ...
        timestep = timestep.to(model_dtype) * 1000
        ...
        if kv_cache_mode == "extract" and num_ref_tokens > 0:
            ...
            self.time_guidance_embed(ref_timestep, guidance)

        # 2. Input projection for image (hidden_states) and conditioning text (encoder_hidden_states)
        # IMPORTANT: Cast to model dtype BEFORE x_embedder to avoid dtype mismatch
