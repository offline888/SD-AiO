...
class Flux2KleinIRPipeline(Flux2KleinPipeline):
    r"""
    The Flux2 Klein pipeline for text-to-image generation.
    ...
    Args:
        ...
        tokenizer (`Qwen2TokenizerFast`):
            ...
            [Qwen2TokenizerFast](https://huggingface.co/docs/transformers/en/model_doc/qwen2#transformers.Qwen2TokenizerFast).
    """
    # modified pipeline for one-step image restoration
    def __init__(
        self,
        scheduler: FlowMatchEulerDiscreteScheduler,
        vae: AutoencoderKLFlux2,
        ...
        super().__init__(scheduler, vae, text_encoder, tokenizer, transformer, is_distilled)
    ...
    def __call__(
        self,
        lq_image: list[PIL.Image.Image] | PIL.Image.Image | None = None,
        prompt: str | list[str] = None,
        height: int | None = None,
        width: int | None = None,
        ...
        max_sequence_length: int = 512,
        text_encoder_out_layers: tuple[int] = (9, 18, 27),
        fixed_timestep: int = 300,
    ):
        r"""
        Function invoked when calling the pipeline for generation.

        Args:
            lq_image (`PIL.Image.Image` or `list[PIL.Image.Image]`):
                Low-quality input image(s) for image restoration.
            prompt (`str` or `list[str]`, *optional*):
            ...
            output_type (`str`, *optional*, defaults to `"pil"`):
                The output format: `"pil"` or `"latent"`.
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether to return a Flux2PipelineOutput instead of a plain tuple.
            attention_kwargs (`dict`, *optional*):
                Optional kwargs passed to the AttentionProcessor.
            callback_on_step_end (`Callable`, *optional*):
