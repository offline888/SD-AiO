...
OUT_DIR="output/ablation_roundrobin/t250_${TIMESTAMP}"
...
echo "=== RR t=250 + simple-conv + VAE LoRA ==="
accelerate launch --num_processes=2 --mixed_precision=bf16 src/train.py     --data_config configs/tasks_3d_roundrobin.yaml --output_dir "${OUT_DIR}"     --sd_path /root/shared-nvme/model/sd2-1 --condition_type deg-aware     --backbone_type simple-conv --timestep_value 100     --dino_type /root/shared-nvme/model/dinov2     --degradation_classifier_path "${CLASSIFIER}"     --enable_lora --lora_rank_vae 16 --round_robin     --timestep_value 250     --lambda_l2 2.0 --lambda_lpips 5.0 --learning_rate 1e-4     --lr_scheduler cosine --lr_warmup_steps 500 --adam_weight_decay 0.0     --max_grad_norm 1.0 --gradient_accumulation_steps 16 --max_train_steps 5000     --mixed_precision bf16 --seed 42 --checkpointing_steps 2000 --eval_freq 500     --num_images_save_eval 15 > "${OUT_DIR}/train.log" 2>&1
echo "=== Done (exit=$?) ==="
