...
OUT_DIR="output/ablation_roundrobin/convnext_dinoft_${TIMESTAMP}"
...
echo "=== RR t=100 + convnext_tiny + VAE LoRA + DINO LPIPS ==="
accelerate launch --num_processes=2 src/train.py \
    ...
    --lambda_l2                   2.0 \
    --lambda_lpips                250 \
    --lpips_layers               1,2,3,4 \
    --dino_lpips_ckpt            output/stage1_3d_focal_20260620_172503/best_model.pth \
    --learning_rate               1e-4 \
    --lr_scheduler                cosine \
    --lr_warmup_steps             500 \
