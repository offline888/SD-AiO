...
def log_once(message, accelerator=None):
    global _printed
    if id(message) in _printed:
        return
    _printed.add(id(message))
...
class FLUX2ModulationV2(nn.Module):
    ...
    def fourier_encode(
        self, block_idx: int | torch.Tensor, target_dtype: torch.dtype = torch.bfloat16
    ) -> torch.Tensor:
    ...
    def forward(
        ...
        lq_tensor = lq_tensor.to(temb.dtype)
        ...
        if block_idx is not None:
            block_emb = self.block_proj(
                self.fourier_encode(block_idx, target_dtype=temb.dtype)
            )
...
class DegFeatExtractor:
    def __init__(
        ...
        class_embedding_U: torch.Tensor = None,
    ):
        self.args = args
        ...
        if class_embedding_U is not None:
            self._class_embedding_U = class_embedding_U.to(
                device=device, dtype=weight_dtype
            )
            self._class_embedding_U.requires_grad_(False)
            print(
                f"[DegFeatExtractor] Loaded frozen class_embedding_U, shape={self._class_embedding_U.shape}"
            )
        else:
            self._class_embedding_U = nn.Parameter(
                torch.randn(
                    num_deg_types, inner_dim, device=device, dtype=torch.float32
                )
            )
            nn.init.orthogonal_(self._class_embedding_U)
            with torch.no_grad():
                self._class_embedding_U.copy_(self._class_embedding_U.to(weight_dtype))
            transformer.register_parameter("class_embedding_U", self._class_embedding_U)
            print(
                f"[DegFeatExtractor] Registered trainable class_embedding_U, shape={self._class_embedding_U.shape}"
            )

        self.deg_classifier = None
        if args.degradation_classifier_path is not None:
            print("[DegFeatExtractor] Loading degradation classifier...")
            self.deg_classifier = DegNet_DINO(
                dino_type=args.dino_type, num_types=num_deg_types
            )
            ...
            print("[DegFeatExtractor] Degradation classifier loaded successfully")
    ...
    def __call__(self, lq_images: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            embedding = self._class_embedding_U
            if embedding.device != probs.device or embedding.dtype != self.weight_dtype:
                embedding = embedding.to(device=probs.device, dtype=self.weight_dtype)
        deg_feat = probs.to(dtype=self.weight_dtype) @ embedding  # [B, inner_dim]
        return deg_feat
...
def main(args):
    ...
    deg_extractor = DegFeatExtractor(
        ...
        args=args,
        class_embedding_U=None,
    )
    # Inference mode: pass trained class_embedding_U from checkpoint
    ...
    params_to_optimize = [
        {
            "params": [
                ...
                if p.requires_grad and p is not deg_extractor._class_embedding_U
            ],
