...
bsz = 1
C, H, W = 16, 32, 32
lq = torch.randn(bsz, C, H, W, device=device, dtype=torch.float32)
hq = torch.randn(bsz, C, H, W, device=device, dtype=torch.float32)
...
try:
    ...
    print(f"VAE 加载失败 (可能需要 GPU): {e}")
    print("\n使用模拟的 BN 统计量 (bn_mean=0, bn_std=1) 进行验证")
    bn_mean = torch.zeros(1, 16, 1, 1)
    bn_std = torch.ones(1, 16, 1, 1)
